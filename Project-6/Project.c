...
typedef enum block_type{start=0,property,fortune,tax,punish,}block_type;
...
void sell_property(block board[20], player* current_player)
{
        ...
        if (count_properties(*current_player)!=0)
        {
            ...
            if (property_id!=0)
            {
                ...
                if(is_owned!=-1)
                {
                    ...
                    if(board[property_id].house_count==0)current_player->account+=board[property_id].price/2;
                    if(board[property_id].house_count==1)
                    {
                        current_player->account+=(board[property_id].price+board[property_id].house_price)/2;
                    }
                    if(board[property_id].house_count==2)
                    {
                        current_player->account+=(board[property_id].price+(2*board[property_id].house_price))/2;
                    }
                    if(board[property_id].house_count==3)
                    {
	...
	if(current_player->type==cap){
	        if (count_properties(*current_player)!=0)
	        {
	        	property_id=board[current_player->owned_blocks[0]].price;

	            for (i = 1; i < PROPERTY_COUNT; i++) {
	                if (current_player->owned_blocks[i] != -1) {
	                	//property_id is the max value of the prices that computer owns
	                	if(board[current_player->owned_blocks[i]].price>property_id)
	                		property_id=board[current_player->owned_blocks[i]].price;
	                    	   	counter++;
	                }
	        ...
	        {
	            printf("Computer doesn't have any property to sell.\n");
	        }
	    }
...
void pay_rent(block board[20], player* current_player,player * owner)
{
    ...
    if (board[current_player->current_block_id].house_count==1)rent=board[current_player->current_block_id].rent_1;
    if (board[current_player->current_block_id].house_count==2)rent=board[current_player->current_block_id].rent_2;
    if (board[current_player->current_block_id].house_count==3)rent=board[current_player->current_block_id].rent_3;
...
void buy_property(block* current_block, player* current_player)
{
    if (current_block->price<=current_player->account)
    {
    	...
    	if(current_player->type==cap){
	        ...
	        printf("Computer's current account is: %d$ \n",current_player->account);
    	}
    }
    else
...
{
    int house_flag=1,selection,insuf_flag=1,build_flag=1,dice;
    if(current_player->type==car){
	...
	if(current_player->type==cap){
		while (house_flag && build_flag) {
		 	if(count_properties(*current_player)>=12/3){
                    dice=roll_dice();
                if(dice<=3 && dice>=1)
                    selection=1;
                else{
...
{
    int selection,insuf_flag=1,i,sum=0,dice,rand,a;
    if (current_player->type==car){
	    switch (board[current_player->current_block_id].type)
	    {
	        ...
	        case fortune:
	        	rand=random_fortune();
	        		if(rand==1){
	        			printf("You picked the free house card!\n");
	        			a=current_player->account;
	        			build_house(&board[current_player->current_block_id], current_player);
	        			current_player->account=a;
	        		}
	        		else if(rand==2){
	        			printf("You picked the time travel card!\n");
	        			dice=roll_dice();
	        			if(dice<=3 && dice>=1)
	    ...
	    switch (board[current_player->current_block_id].type)
	    {
	        case property:
	            ...
	            if (board[current_player->current_block_id].owner.type==none)
	            {
	                for(i=0;i<20;i++){
		               	if (board[i].type==property)
		               	sum=board[i].price+sum;
		            }
               			sum=sum/12;
	               if(sum>=board[current_player->current_block_id].price)
	               		selection=1;
	               else
	               		dice=roll_dice();
